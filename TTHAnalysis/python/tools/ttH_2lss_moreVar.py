...
class Class_ttH_2lss_moreVar(Module):
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        for jesLabel in self.systsJEC.values():
            ...
            self.out.branch('%shiggs_reco_pt%s'%(self.label,jesLabel), 'F')
    ...
    def setDefault(self, event, jesLabel):
        ...
        self.out.fillBranch('%shiggs_reco_pt%s'%(self.label,jesLabel), -99.)
        # Gen level, the labels
...
(j1_q+j2_q).Pt())
            ...
            for iLep in range(self.nlep):
                ...
                self.out.fillBranch('%sClosestJetToLep_pt%s%s'%(self.label,iLep,jesLabel) , lep_closestJet[iLep].Pt() if iLep < len(lep_closestJet) else -99)            # Fill MET and GEN observ\ables
